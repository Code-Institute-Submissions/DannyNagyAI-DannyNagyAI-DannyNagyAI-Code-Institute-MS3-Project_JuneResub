...
db = client.iSandwichDB
sandwich_collection = db.sandwiches
...
def index():
    return render_template(
        ...
        subtitle="Heal your cravings",
        heroimage="coverphoto-bg1.jpg",
    )
...
def test():
    return render_template(
        "test.html",
        title="Test layout",
        subtitle="Please give us peace",
        heroimage="coverphoto-bg6.jpg",
    )
...
def search_sandwiches():
    ...
    search_result = sandwich_collection.find(
        {"ingredients": {"$in": search_string}}
    )
    return render_template(
        "sandwiches.html", search_result=search_result,
        heroimage="coverphoto-bg7.jpg"
    )
...
def view_sandwich():
    ...
    sandwich = sandwich_collection.find_one(
        {"_id": bson.ObjectId(oid=str(id))}
    )
    return render_template(
        "sandwich.html",
        sandwich=sandwich,
        title=sandwich.get("sandwich_name"),
        subtitle="Check it out dude!",
        heroimage="coverphoto-bg14.jpg",
    )
...
def update_sandwich():
    ...
    user_sandwiches = sandwich_collection.find(
        {"user_name": session["user_name"]}
    )

    return render_template(
        "profile.html",
        user_sandwiches=user_sandwiches,
        title="Profile",
        subtitle=session["user_name"],
        heroimage="coverphoto-bg3.jpg",
    )
...
def sandwiches():
    ...
    return render_template(
        "sandwiches.html",
        sandwiches=sandwiches_1,
        categories=catogories_1,
        title="Sandwiches",
        subtitle="Take a look at our masterchef recipes",
        heroimage="coverphoto-bg7.jpg",
    )
...
def login_form():
    return render_template(
        "login_form.html",
        title="Log in",
        subtitle="Welcome to the pleasuredome!",
        heroimage="coverphoto-bg3.jpg",
    )
...
def login():
    ...
    if user is not None:
        # User is now loged in, setting some session variables
        # (might wanna replace later on)
        session["display_name"] = user.get("display_name")
        session["user_name"] = user.get("user_name")

        # We also want to provide the profile page
        # with the current loged in users sandwiches
        user_sandwiches = sandwich_collection.find(
            {"user_name": user.get("user_name")}
        )

        # Then we send the loged in user to his profile page
        # with his sandwiches to go :-)
        return render_template(
            "profile.html",
            user_sandwiches=user_sandwiches,
            title="Profile",
            subtitle=session["display_name"],
            heroimage="coverphoto-bg3.jpg",
        )
    else:
...
def logout():
    ...
    return render_template(
        ...
        message=message,
        title="Log out",
        subtitle="See you later.. miss u allready",
        heroimage="coverphoto-bg3.jpg",
    )
...
def signup_form():
    return render_template(
        "signup_form.html",
        title="Sign up",
        subtitle="Sharing is caring - come on, join now!",
        heroimage="coverphoto-bg1.jpg",
    )
...
def signup():
    ...
    if users_collection.find_one({"user_name": user_name}) is not None:
        ...
        message = (
            "The registration went well, user " +
            display_name + " is now a member"
        )
        return render_template("signup_form.html", message=message)
...
def sandwich_form():
    ...
    return render_template(
        "sandwich_form.html",
        categories=categories,
        title="Add a sandwich",
        subtitle=session["user_name"],
        heroimage="coverphoto-bg12.jpg",
    )
...
def add_sandwich():
    ...
    # file upload image to our folder of sandwich images
    # --------------------------------------------------------------------------
    ...
    # 3. Send the user back to profile page
    # with a success-message and a list of the users sandwiches
